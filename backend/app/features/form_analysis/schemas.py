...
class BiomechanicsMetrics(BaseModel):
    ...
    shoulderHeight: float = Field(
        ..., ge=0, le=1, description="Shoulder height normalized (0-1)"
    )
    kneeForwardTravel: float = Field(
        ..., ge=-100, le=100, description="Knee forward travel as percentage"
    )
...
class TemporalAnalysis(BaseModel):
    ...
    riseRateRatio: float = Field(
        ..., ge=0, description="Ratio of hip rise to shoulder rise"
    )
    maxTorsoLean: float = Field(
        ..., ge=-90, le=90, description="Maximum torso lean angle in degrees"
    )
    maxKneeForwardTravel: float = Field(
        ..., ge=-100, le=100, description="Maximum knee forward travel percentage"
    )
    neckExtensionMax: float = Field(
        ..., ge=0, le=180, description="Maximum neck extension angle"
    )
    minHipAngle: float = Field(
        ..., ge=0, le=180, description="Minimum hip angle (indicates depth)"
    )
...
class FormAnalysisRequest(BaseModel):
    ...
    exerciseType: str = Field(
        ...,
        min_length=1,
        max_length=50,
        description="Type of exercise (e.g., 'Squat', 'Deadlift')",
    )
    frameCount: int = Field(..., ge=1, le=100, description="Number of frames analyzed")
    duration: str = Field(
        ..., max_length=100, description="Approximate duration of movement"
    )
    keyPositions: KeyPositions = Field(..., description="Key positions in the movement")
    temporalAnalysis: TemporalAnalysis = Field(
        ..., description="Temporal movement patterns"
    )
    riskFlags: list[str] = Field(
        default=[], max_length=20, description="Automatically detected risk flags"
    )
    allFramesData: list[BiomechanicsMetrics] | None = Field(
        None,
        max_length=100,
        description="Frame-by-frame metrics (optional, not used in analysis)",
    )
