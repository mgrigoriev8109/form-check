...
describe('VideoUploader', () => {
  ...
  describe('URL Sanitization - XSS Protection', () => {
    it('should sanitize blob URL before using it', async () => {
      const user = userEvent.setup();
      render(<VideoUploader onVideoSelected={mockOnVideoSelected} />);
      ...
      // Verify that the URL created is a blob URL
      expect(mockVideo.src).toMatch(/^blob:/);
    });

    it('should block javascript: protocol if somehow injected via URL.createObjectURL', async () => {
      const user = userEvent.setup();

      // Mock URL.createObjectURL to return a malicious URL
      ...
      // Should NOT have javascript: URL
      expect(mockVideo.src).not.toContain('javascript:');
      expect(mockVideo.src).not.toContain('alert');
    });

    it('should block data: protocol if somehow injected', async () => {
      const user = userEvent.setup();

      // Mock URL.createObjectURL to return a data URL
      ...
      // Should NOT have data: URL
      expect(mockVideo.src).not.toContain('data:');
      expect(mockVideo.src).not.toContain('script');
    });

    it('should block http: URLs if somehow injected', async () => {
      const user = userEvent.setup();

      // Mock URL.createObjectURL to return an http URL
      ...
      // Should NOT have the external URL
      expect(mockVideo.src).not.toContain('evil.com');
    });
  });
